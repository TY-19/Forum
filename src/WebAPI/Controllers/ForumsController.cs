...
using Forum.Application.Forums.Queries.GetForumsStructure;
...
public class ForumsController(IMediator mediator) : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<ForumStructureDto>>> GetForumsStructure(CancellationToken cancellationToken)
    {
        return Ok(await mediator.Send(new GetForumsStructureRequest(), cancellationToken));
    }
